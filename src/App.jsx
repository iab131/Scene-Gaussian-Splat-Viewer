...
export default function app() {
  ...
  useEffect(() => {
    ...
    sceneRef.current = scene; // Save it to the ref!

    const geometry = new THREE.BoxGeometry(1, 1, 1);
    const material = new THREE.MeshBasicMaterial({ color: 0x00ff00, wireframe: true });
    const cube = new THREE.Mesh(geometry, material);
    cube.position.set(0, 0, 0);
    sceneRef.current.add(cube); // If you see a green cube, the renderer is fine!
    ...
    camera.lookAt(0, 0, 0);
    //camera.up.set(0, -1, 0); // Correctly set up-vector for inverted .ply files
    ...
    selfDrivenMode: false, // You are the driver now
    useBuiltInControls: false, // You are using OrbitControls
    renderer: renderer,
    camera: camera,
    threeScene: scene,
    showControlPlane: false, // Removes the white circle
    gpuAcceleratedSort: false
});
viewerRef.current = viewer;
    
    window.viewer = viewer; // 👈 add this
    ...
    // Resize Handler
    const handleResize = () => {
    if (!cameraRef.current || !rendererRef.current || !viewerRef.current) return;
    ...
    // 👈 CRITICAL: Tell the splat viewer about the new size
...
const performAutoFrame = (object) => {
    ...
    // Use the library's built-in bounding box logic if possible
    const box = new THREE.Box3().setFromObject(object);
    
    if (box.isEmpty() || isNaN(box.min.x)) {
        console.warn("Box is empty, defaulting camera.");
    ...
    let cameraZ = Math.abs(maxDim / 2 / Math.tan(fov / 2)) * 2.0; // 2x padding

    cameraRef.current.position.set(center.x, center.y + (maxDim * 0.2), center.z + cameraZ);
...
const loadFile = async (file) => {
    ...
    try {
        ...
        // 2. Add to scene (if not already there)
        ...
        // 3. Wait a few frames for the GPU to populate buffers before framing
        setTimeout(() => {
...
const frameScene = () => {
  ...
  return (
    ...
    >
      ...
      {isDragging && (
         <div className="absolute inset-0 z-50 flex items-center justify-center bg-cyan-500/20 backdrop-blur-sm border-4 border-cyan-400 border-dashed m-4 rounded-xl pointer-events-none">
      ...
      {!hasScene && !loading && !isDragging && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-[#001524]/80 backdrop-blur-sm">
      ...
      {/* Stats Panel */}
      <div className="absolute top-4 left-4 z-10 p-4 bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-lg shadow-xl min-w-[200px]">
      ...
      {/* Controls */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex gap-4 p-2 bg-slate-900/80 backdrop-blur-md rounded-full border border-slate-700/50 shadow-2xl pointer-events-auto">
        <button
          onClick={resetView}
          disabled={!hasScene}
